...
- [x] P0-8：人格 T 恤承接、颜色/尺码选择、¥129 人格认证价、旧微信支付/退款能力兼容；数据库启用后先锁 SKU 并事务化预留库存，支付成功回调幂等转为已售库存，失败释放预留。
...
4. **真实支付全链路测试**：微信 OAuth → JSAPI/NATIVE 下单 → SKU 预留 → 支付回调 → 库存扣减 → Dashboard → 退款必须在真实商户环境逐项验证。
5. **真实商品数据**：缺失时页面保持 `NEED_REAL_PHOTO / NEED_REAL_PRODUCT_DATA`，禁止用假 UGC 或虚构参数替代。
6. **算法产品验收**：12 题逐选项人格权重必须由产品侧确认后冻结。

## 安全处理

- `.env`、商户私钥、证书、订单 JSON、统计 NDJSON、生成卡片 PNG 均不会提交 Git。
- `storage/.htaccess` 阻止 JSON/NDJSON/日志被网页直接读取，生成的人格卡 PNG 可公开访问。
- 管理订单和 Dashboard 接口要求 `X-Admin-Token`。
- 微信支付/退款回调校验时间戳、平台证书序列号、RSA-SHA256 签名后才解密业务数据。
- 订单数据库使用 InnoDB 事务 + `SELECT ... FOR UPDATE` 锁 SKU，避免并发超卖；支付成功回调按订单状态保证幂等扣减。
- CI 检查 PHP/JS/JSON 语法，并阻止典型密钥/运行文件进入仓库。

## 当前结论

P0 的主要代码结构已经齐全，但仍是 **Draft / 待真实环境验收**。当前不能直接替换生产站，主要剩余工作是服务器环境、真实商品/库存数据、微信商户全链路和算法映射验收。
